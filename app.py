...
<style>
    ...
    [data-testid="stSidebar"] [data-testid="stFileUploader"] small { color: #a8b8d8 !important; }
...
def uploaded_to_bytes(uploaded_file):
    """UploadedFile → BytesIO"""
    uploaded_file.seek(0)
    return BytesIO(uploaded_file.read())

# ─────────────────────────────────────────────
# xls → xlsx 메모리 변환 (xlrd 사용)
# ─────────────────────────────────────────────
def xls_bytes_to_xlsx_bytes(file_bytes: BytesIO) -> BytesIO:
    """xls BytesIO → xlsx BytesIO (xlrd + openpyxl)"""
    try:
        import xlrd
        from openpyxl import Workbook as XlsxWorkbook
        file_bytes.seek(0)
        xls_wb = xlrd.open_workbook(file_contents=file_bytes.read())
        new_wb = XlsxWorkbook()
        ...
        for sheet_name in xls_wb.sheet_names():
            ...
            for r in range(xls_ws.nrows):
                for c in range(xls_ws.ncols):
                    new_ws.cell(row=r+1, column=c+1, value=xls_ws.cell(r, c).value)
        out = BytesIO()
        new_wb.save(out)
        out.seek(0)
        return out
    except ImportError:
        st.error("❌ xlrd 패키지가 없습니다. 터미널에서 `pip install xlrd` 를 실행하세요.")
        st.stop()

# ─────────────────────────────────────────────
# 데이터 로드 함수 (BytesIO 기반)
# ─────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def load_billing_data(file_bytes: bytes):
    """청구금액_및_삭감내역 xlsx bytes → dict"""
...
@st.cache_data(show_spinner=False)
def load_prescription_data(file_bytes: bytes, kind: str = 'outpatient'):
    """처방 xlsx bytes → (월별요약, 항목별, 의사별) DataFrames"""
    wb = load_workbook(BytesIO(file_bytes), read_only=True)
    ...
    for sh in wb.sheetnames:
        ...
        for row in rows[2:]:
            ...
            if item not in item_records:
                item_records[item] = {'총투': 0.0, '금액': 0.0}
        ...
        monthly_summary.append({'월': month_label, '총투': total_qty, '금액': total_amt})
        for item, vals in item_records.items():
            monthly_detail.append({'월': month_label, '항목': item,
                                   '총투': vals['총투'], '금액': vals['금액']})
    ...
    doc_records = [{'의사': doc, '월': mon, '총투': v['총투'], '금액': v['금액']}
                   for (doc, mon), v in doctor_monthly.items()]
    df_doctor = pd.DataFrame(doc_records) if doc_records else pd.DataFrame(
        columns=['의사', '월', '총투', '금액'])
...
아래 3개 파일을 각각 클릭하여 업로드하세요.<br>
① 청구금액 및 삭감내역 <b>(.xlsx)</b><br>
② 외래 처방 내역 <b>(.xls / .xlsx)</b><br>
③ 입원 처방 내역 <b>(.xlsx)</b>
</div>
""", unsafe_allow_html=True)
    ...
    billing_upload = st.file_uploader(
        ...
        type=["xlsx", "xls"],
        key="billing",
        help="2025_청구금액_및_삭감내역.xlsx"
    )
    outpat_upload = st.file_uploader(
        "② 외래 처방 내역",
        type=["xlsx", "xls"],
        key="outpat",
        help="2025처방_외래_.xls 또는 .xlsx"
    )
    inpat_upload = st.file_uploader(
        "③ 입원 처방 내역",
        type=["xlsx", "xls"],
        key="inpat",
        help="2025처방_입원_.xlsx"
    )
...
# 업로드 전 안내 화면
# ─────────────────────────────────────────────
all_uploaded = billing_upload and outpat_upload and inpat_upload
if not all_uploaded:
    missing = []
    if not billing_upload: missing.append("① 청구금액 및 삭감내역 파일")
    if not outpat_upload:  missing.append("② 외래 처방 내역 파일")
    if not inpat_upload:   missing.append("③ 입원 처방 내역 파일")

    st.info(f"👈 왼쪽 사이드바에서 파일을 업로드하면 분석이 시작됩니다.")
    st.markdown("#### 📋 아직 업로드되지 않은 파일")
    for m in missing:
        st.warning(f"⬆️ {m} 을(를) 업로드해주세요")
    ...
    | 파일 | 내용 |
    |------|------|
    | `2025_청구금액_및_삭감내역.xlsx` | 건강보험·의료급여 외래·입원 청구·조정 내역 |
    | `2025처방_외래_.xls` | 월별 외래 처방 항목·의사별 내역 |
    | `2025처방_입원_.xlsx` | 월별 입원 처방 항목·의사별 내역 |
    """)
    st.stop()
...
with st.spinner("📊 데이터를 분석하는 중입니다..."):
    # 청구/삭감 데이터
    billing_bytes = uploaded_to_bytes(billing_upload).read()
    billing_data  = load_billing_data(billing_bytes)

    # 외래 처방 (xls이면 변환)
    outpat_io = uploaded_to_bytes(outpat_upload)
    if outpat_upload.name.lower().endswith('.xls'):
        outpat_io = xls_bytes_to_xlsx_bytes(outpat_io)
    outpat_bytes = outpat_io.read()

    # 입원 처방
    inpat_bytes = uploaded_to_bytes(inpat_upload).read()

    df_out_sum, df_out_det, df_out_doc = load_prescription_data(outpat_bytes, 'outpatient')
    df_inp_sum, df_inp_det, df_inp_doc = load_prescription_data(inpat_bytes,  'inpatient')

    # 월 필터
    df_out_sum_f = df_out_sum[df_out_sum['월'].isin(sel_months)] if not df_out_sum.empty else df_out_sum
    df_inp_sum_f = df_inp_sum[df_inp_sum['월'].isin(sel_months)] if not df_inp_sum.empty else df_inp_sum
    df_out_det_f = df_out_det[df_out_det['월'].isin(sel_months)] if not df_out_det.empty else df_out_det
    df_inp_det_f = df_inp_det[df_inp_det['월'].isin(sel_months)] if not df_inp_det.empty else df_inp_det

    # 청구 데이터 헬퍼
    def get_billing(sheet, kind):
        df = billing_data.get((sheet, kind), pd.DataFrame())
        if not df.empty:
...
st.success(f"✅ 데이터 로드 완료 — {billing_upload.name} / {outpat_upload.name} / {inpat_upload.name}")
...
with k4:
    ...
    <div class="kpi-card purple">
        ...
        <div class="kpi-value">{fmt_won(out_rx_total + inp_rx_total)}</div>
...
with tab1:
    ...
    with col_sel[0]: bill_type = st.selectbox("보험 구분", ["전체","건강보험","의료급여"])
    with col_sel[1]: care_type = st.selectbox("진료 구분", ["전체","외래","입원"])
    with col_sel[2]: chart_kind = st.selectbox("차트 종류", ["막대+선","누적 막대","영역"])

    def select_bill_df(bt, ct):
        if bt == "전체":
            if ct == "전체": return all_bill_all
            if ct == "외래": return all_bill_out
            return all_bill_inp
        sheet = "건강보험" if bt == "건강보험" else "의료급여"
        if ct == "전체":
            return combine_billing([get_billing(sheet,'outpatient'), get_billing(sheet,'inpatient')])
        return get_billing(sheet, 'outpatient' if ct == "외래" else 'inpatient')
    ...
    if bdf.empty:
        ...
        with m1: st.metric("청구액 합계",   fmt_won(bdf['청구액'].sum()))
        with m2: st.metric("조정액 합계",   fmt_won(bdf['조정액'].sum()),
                           delta=f"-{fmt_won(bdf['조정액'].sum())}", delta_color="inverse")
        with m3: st.metric("평균 조정비율", fmt_pct(bdf['조정비율'].mean()))
        with m4:
            mr = bdf.loc[bdf['조정비율'].idxmax()]
            st.metric("최고 조정비율 월", f"{mr['월']} ({fmt_pct(mr['조정비율'])})")
        ...
        if chart_kind == "막대+선":
            ...
            fig = go.Figure([
                go.Bar(x=bdf['월'], y=bdf['심사결정액'], name='심사결정액', marker_color='#22c55e'),
                go.Bar(x=bdf['월'], y=bdf['조정액'],     name='조정액',    marker_color='#ef4444'),
            ])
            fig.update_layout(barmode='stack', height=420, plot_bgcolor='white',
                              yaxis_title='금액 (원)', legend=dict(orientation='h', y=1.08))
        else:
            fig = go.Figure([
                go.Scatter(x=bdf['월'], y=bdf['청구액'],     name='청구액',    fill='tonexty',
                           fillcolor='rgba(59,130,246,0.2)', line=dict(color='#3b82f6')),
                go.Scatter(x=bdf['월'], y=bdf['심사결정액'], name='심사결정액', fill='tozeroy',
                           fillcolor='rgba(34,197,94,0.2)',  line=dict(color='#22c55e')),
                go.Scatter(x=bdf['월'], y=bdf['조정액'],     name='조정액',    fill='tozeroy',
        ...
        disp = bdf[['월','요양급여총액','건수','청구액','조정액','조정비율','심사결정액']].copy()
        for col in ['청구액','조정액','심사결정액','요양급여총액']:
            disp[col] = disp[col].map(lambda x: f"{x:,.0f}")
        disp['건수']     = disp['건수'].map(lambda x: f"{x:,.0f}")
        disp['조정비율'] = disp['조정비율'].map(lambda x: f"{x:.2f}%")
        st.dataframe(disp, use_container_width=True, hide_index=True)

    st.markdown("---")
    st.markdown("**🔄 건강보험 vs 의료급여 비교**")
    c1, c2 = st.columns(2)
    with c1:
        ...
        if not hb_all.empty:
            ...
            fig2.update_layout(height=320, plot_bgcolor='white', legend=dict(orientation='h', y=1.1))
    ...
    with c2:
        ...
        if not mg_all.empty:
            ...
            fig3.update_layout(height=320, plot_bgcolor='white', legend=dict(orientation='h', y=1.1))
...
with tab2:
    ...
    if rx_tab in ["외래","입원"]:
        ...
        if dfs.empty:
            ...
            c1, c2 = st.columns(2)
            with c1:
                fig = px.bar(dfs, x='월', y='금액', title=f'{rx_tab} 월별 처방 금액',
                             color='금액', color_continuous_scale='Blues')
                fig.update_layout(height=360, plot_bgcolor='white')
                st.plotly_chart(fig, use_container_width=True)
            with c2:
                fig2 = px.bar(dfs, x='월', y='총투', title=f'{rx_tab} 월별 처방 건수(총투)',
            ...
            if not bill_ref.empty:
                merged = dfs.merge(bill_ref[['월','청구액','심사결정액','조정액']], on='월', how='outer').fillna(0)
                fig3 = go.Figure([
                    go.Scatter(x=merged['월'], y=merged['금액'],       name='처방금액',   mode='lines+markers', line=dict(color='#8b5cf6', width=3)),
                    go.Scatter(x=merged['월'], y=merged['청구액'],     name='청구액',     mode='lines+markers', line=dict(color='#3b82f6', width=3)),
                    go.Scatter(x=merged['월'], y=merged['심사결정액'], name='심사결정액', mode='lines+markers', line=dict(color='#22c55e', width=3, dash='dot')),
                ])
        ...
        if df_out_sum_f.empty and df_inp_sum_f.empty:
            ...
            c1, c2 = st.columns(2)
            with c1:
                fig_pie = go.Figure(go.Pie(labels=['외래','입원'],
                    values=[df_out_sum_f['금액'].sum(), df_inp_sum_f['금액'].sum()],
                    hole=0.4, marker_colors=['#3b82f6','#8b5cf6']))
                fig_pie.update_layout(title='처방금액 구성비', height=320)
                st.plotly_chart(fig_pie, use_container_width=True)
            with c2:
                fig_pie2 = go.Figure(go.Pie(labels=['외래','입원'],
                    values=[df_out_sum_f['총투'].sum(), df_inp_sum_f['총투'].sum()],
                    hole=0.4, marker_colors=['#22c55e','#f59e0b']))
...
with tab3:
    ...
    if df_doc.empty:
        ...
        with col1:
            sel_docs   = st.multiselect("의사 선택", doctors_list,
                                        default=doctors_list[:6] if len(doctors_list) > 6 else doctors_list)
        with col2:
        ...
        if sel_docs:
            ...
            fig = px.line(df_doc_sel, x='월', y=doc_metric, color='의사',
            ...
            fig2 = px.bar(doc_agg, x='의사', y=doc_metric,
                          title=f"의사별 총 처방 {doc_metric} 순위",
                          color=doc_metric, color_continuous_scale='Blues', text_auto=True)
            fig2.update_layout(height=360, plot_bgcolor='white')
            st.plotly_chart(fig2, use_container_width=True)
...
with tab4:
    ...
    if df_det.empty:
        ...
        item_agg = (df_det.groupby('항목')[item_metric].sum()
                    .reset_index().sort_values(item_metric, ascending=False).head(top_n))

        c1, c2 = st.columns([2, 1])
        with c1:
            fig = px.bar(item_agg.sort_values(item_metric),
                         x=item_metric, y='항목', orientation='h',
                         title=f"TOP {top_n} 처방 항목 ({item_metric} 기준)",
                         color=item_metric, color_continuous_scale='Blues', text_auto=True)
        ...
        with c2:
            ...
            fig2.update_layout(height=400, showlegend=True, legend=dict(font_size=11))
            st.plotly_chart(fig2, use_container_width=True)
        ...
        if sel_items:
            fig3 = px.line(df_det[df_det['항목'].isin(sel_items)],
                           x='월', y=item_metric, color='항목', markers=True,
                           title=f"선택 항목 월별 {item_metric} 추이",
                           color_discrete_sequence=px.colors.qualitative.Plotly)
            fig3.update_layout(height=380, plot_bgcolor='white', legend=dict(orientation='h', y=-0.2))
            st.plotly_chart(fig3, use_container_width=True)

        st.markdown("---")
        st.markdown("**🗺️ 항목×월 히트맵**")
        pivot_hm = df_det[df_det['항목'].isin(item_agg['항목'].tolist())].pivot_table(
            index='항목', columns='월', values=item_metric, fill_value=0)
...
with tab5:
    ...
    df_syu    = billing_data.get(
        ('건강보험' if syu_sheet=="건강보험" else '의료급여',
    ...
    if df_syu.empty:
        ...
        with c1:
            fig = px.bar(df_syu_sorted, x='월', y='조정액', title="월별 조정액",
                         color='조정비율', color_continuous_scale='RdYlGn_r', text_auto=True)
            fig.update_layout(height=380, plot_bgcolor='white')
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = go.Figure([
                go.Bar(x=df_syu_sorted['월'], y=df_syu_sorted['청구액'], name='청구액', marker_color='#3b82f6'),
                go.Bar(x=df_syu_sorted['월'], y=df_syu_sorted['조정액'], name='조정액', marker_color='#ef4444'),
                go.Scatter(x=df_syu_sorted['월'], y=df_syu_sorted['조정비율'],
                           name='조정비율(%)', yaxis='y2',
            ...
            fig2.update_layout(
                ...
                yaxis2=dict(title='조정비율 (%)', overlaying='y', side='right',
                            range=[0, max(df_syu_sorted['조정비율'].max()*1.5, 5)]),
...
with tab6:
    ...
    data_sel = st.selectbox("데이터 선택", [
        "청구/삭감 - 건강보험 외래","청구/삭감 - 건강보험 입원",
        "청구/삭감 - 의료급여 외래","청구/삭감 - 의료급여 입원",
        "처방 - 외래 월별 합계","처방 - 입원 월별 합계",
        "처방 - 외래 항목별","처방 - 입원 항목별",
        "처방 - 외래 의사별","처방 - 입원 의사별",
    ])
    data_map = {
        ...
        "처방 - 외래 월별 합계": df_out_sum,  "처방 - 입원 월별 합계": df_inp_sum,
        "처방 - 외래 항목별":   df_out_det,  "처방 - 입원 항목별":   df_inp_det,
        "처방 - 외래 의사별":   df_out_doc,  "처방 - 입원 의사별":   df_inp_doc,
    }
    ...
    if show_df.empty:
        ...
        st.download_button(
            label="📥 CSV 다운로드",
            data=show_df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'),
